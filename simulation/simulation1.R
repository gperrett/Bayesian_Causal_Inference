...
# Fit Lm
model_lm <- lm(y~a+b+c+d+e+f+g+z, dat)

# Fit IPTW
iptw <- glm(z ~ a + b + c + d + e + f + g , family = "binomial", data = dat)
...
model_iptw <-lm(y~z + a + b + c + d + e + f + g, weights = wgt, data = dat)

# Fit RF
rf <- ranger(z ~ a + b + c + d + e + f + g, data = dat,mtry = 2, classification = T,min.node.size = 10, num.trees = 100, splitrule= 'gini')
pscores <- rf$predictions
match <- matching(dat$z, pscores, replace = T)
match.wgt <- match$cnts
model_rf <- lm(y~z + a + b + c + d + e + f + g,weights = match.wgt, data= dat )
# Fit BART
X <- as.matrix(dat[,c("a", "b","c","d", "e", "f", "g")])
...
study.1 <- mclapply(1:1000, function(i){simulation1()}, mc.cores = 10)
df1 <- bind_rows(study.1)
...
dat
cor(norms$a, norms$b)
plot(density(inv.log(norms$a)))
dat %>% group_by(z) %>% summarise(mean(c))
plot(inv.log(z))

dat %>% 
  pivot_longer(9:11) %>% 
  ggplot(aes(value, fill = factor(z))) + 
    geom_density(alpha = .5)+
    facet_wrap(~name)
names(dat)
dat %>% 
  pivot_longer(1:4) %>% 
  ggplot(aes(value, colour = factor(z))) + stat_ecdf() +
  facet_wrap(~name)
